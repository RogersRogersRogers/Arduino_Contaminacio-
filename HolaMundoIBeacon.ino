...
const int pinSensorRef = 28;
const int pinSensorGas = 29;
// --------------------------------------------------------------
// --------------------------------------------------------------
namespace Globales {
...
//---------------------------------------------------------------
int funcion(){
  int valorSensorRef = analogRead(pinSensorRef);
  int valorSensorGas = analogRead(pinSensorGas);

  float Vgas = valorSensorGas*(3.3 / 1023.0);
  float Vref = valorSensorRef*(3.3 / 1023.0);

  float Cgas = (Vgas - Vref) /( 6.20 * 494 * pow(10, -6));

  int CgasInt = (int) Cgas *10;
  Serial.println(valorSensorRef);
  Serial.println(valorSensorGas);
  Serial.println(CgasInt);
  return 0;
}
// --------------------------------------------------------------
// --------------------------------------------------------------
